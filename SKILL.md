...
This deliverable contains all static assets (characters, scenes, props) and storyboards with bilingual prompts (English + Chinese) formatted under the **GlitterPixely Standard Layout (Character sheets as turnarounds, storyboards as film panels with technical annotations)** for `image2`, and chronological <=15s video segment prompts for `seedance2.0` that leverage All-Round Reference (全能参考) to ensure absolute character, scene, prop, and pacing consistency.
...
- **Bilingual image2 Prompts**: Every single character, scene, prop, and storyboard frame generated by the user in `image2` must have **Bilingual Prompts (English for maximum model fidelity + Chinese for user tweaking)**.
- **GlitterPixely Layout Standard**: Fixes the layout and format of image2 outputs. Character sheets must be formatted as **orthographic turnaround sheets** (front/side/back view with light-grey background). Storyboards must be formatted as **annotated cinematic panels** (including focal length and camera type annotations).
...
`Upload Video/Link -> Internal 10-Layer Deconstruction & Migration -> Output a Single .md Deliverable (image2 bilingual assets & storyboards under the GlitterPixely standard + seedance2.0 <=15s prompts).`
...
- [backend-handover-protocol.md](references/backend-handover-protocol.md) for image2 JSON formats, GlitterPixely specs, and seedance2.0 prompt parameter requirements.
...
- **Must provide bilingual prompts (English + Chinese)** for every storyboard frame, structured as cinematic panels with technical camera and lens annotations.
