...
3. 工业级脚本审查
   ...
   **v2.0 新增**: 运镜摄影审查岗位（camera arc analysis + mirror symmetry check）。
...
6. Seedance 2.0 提示词生产（v2.0 运镜增强）
   根据脚本、资产和时长，为每个段落生成精细化提示词，尽量锁定人物、服装、场景、光影、空间和镜头连续性。
   **v2.0 核心**: 每段提示词内置专业运镜设计块——从 38 种运镜技巧中精准匹配 1-3 种，并写入速度/距离/焦段/情绪目标等完整参数。
...
- 导演总审
- 编剧审查
- **摄影/运镜审查（v2.0 新增）** — 运镜弧线分析 + 镜像对称检查 + 运镜情绪匹配度评分
- AI 可执行性审查
- 美术风格审查
- 服化道审查
- 后期声音审查
...
- 三个创意方向提案
- 多轮修订后的通过版脚本
- 审查汇总与评分结果（**含运镜审查评分 v2.0**）
- 制片主任 handoff 文档
- Banana 2 角色资产 JSON
- Banana 2 场景资产 JSON
- Banana 2 道具资产 JSON
- 分镜与时间轴文档
- Seedance 2.0 分段提示词包（**v2.0 运镜增强版，含 🎥 运镜设计块 + ✅ 运镜自检**）
- **运镜设计总表（v2.0 新增）**
  - 附录A：38种运镜 × 全SEG映射矩阵
  - 附录B：运镜情绪曲线图
  - 附录C：镜像对称检查报告
...
- `SKILL.md` — 技能入口文件（v2.0 运镜增强）
- `agents/openai.yaml`
- `references/`
  - `cinematography-design-guide.md` — **v2.0 新增** 38种运镜匹配框架
  - `seedance-prompt-craft-notes.md` — **v2.0 升级** 运镜术语从13→38种 + 运镜叙事设计原则
  - `seedance-segment-template.md` — **v2.0 新增** 含🎥运镜设计块的段落模板
  - `seedance-package-output-template.md` — **v2.0 新增** 含运镜总表的输出模板
  - `review-prompts/cinematography-review.md` — **v2.0 新增** 运镜审查提示词
  - [其余原有参考文件...]
- `assets/`
